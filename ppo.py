...
class PPO:
	...
	def rollout(self):
		...
		batch_acts = torch.tensor(batch_acts, dtype=torch.float32).to(self.device)
	...
	def get_action(self, obs):
		# Query actor network for action probabilities
	...
	def evaluate(self, batch_obs, batch_acts):
		# Predicted values from the critic
		V = self.critic(batch_obs).squeeze()

		# Calculate the log probabilities of batch actions using the actor network
		action_probs = torch.softmax(self.actor(batch_obs), dim=-1)
		dist = torch.distributions.Categorical(action_probs)
